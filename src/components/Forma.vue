    ...
    <q-card class="my-card text-center" style="width: 80%">
      ...
      <q-card-section>
        ...
        <h6>
          {{
            language === "pl"
              ? "Proszę wybrać cykl kształcenia"
              : "Please choose training cycle"
          }}
        </h6>
        <div v-if="options" class="row justify-center">
          <div style="min-width: 200px">
            <q-select
              ...
              v-model="cykl"
              :options="options"
              :label="language === 'pl' ? 'Cykl kształcenia' : 'Training cycle'"
              class="q-mb-md"
            />
          </div>
        </div>
...
export default {
  ...
  watch: {
    cykl: function () {
      if (this.cykl) {
        this.$router.push({
          path: this.$route.path + "/" + this.cykl,
        });
      }
    },
    language: function () {
