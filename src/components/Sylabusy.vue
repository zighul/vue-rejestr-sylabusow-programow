      ...
      <q-card-section>
        <div class="q-pa-md q-gutter-sm">
          <q-breadcrumbs>
            ...
            <q-breadcrumbs-el
              :label="
                language === 'pl' && lastPart[2] === 'sylabusy'
                  ? 'Sylabusy'
                  : 'Programy'
              "
            />
        ...
        <div class="row justify-center">
          <div style="min-width: 200px">
            <q-select
              ...
              v-model="wydzial"
              :options="language === 'pl' ? options : options"
...
export default {
  ...
  watch: {
    wydzial: function () {
      if (this.wydzial) {
        this.$router.push({
          path: this.$route.path + "/" + this.routes[this.wydzial],
        });
      }
    },
    language: function () {
  ...
  mounted() {
    axios
      .get(
        // `https://test.umw.edu.pl/json/wydzialy/`
        `http://localhost:3000/wydzialy`
      )
      .then((response) => {
